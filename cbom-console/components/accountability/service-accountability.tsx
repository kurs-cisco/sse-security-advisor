...
function PlanningCell({ plan, label }: { plan: PlanningSummary; label: string }) {
  ...
  const isVendorDependency = plan.entries.some((entry) => entry.raw_value?.trim().toLowerCase() === "vendor dependency");
  if (isDone) return <Badge tone="success">DONE</Badge>;
  if (isVendorDependency) return <Badge tone="warning">VENDOR DEPENDENCY</Badge>;
